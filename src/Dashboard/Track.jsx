...
import ColumnDataTable from './MainTable';
  ...
  const bulkDeleteLoad = async () => {
    ...
    try {
        const response = await axios.put(
            'http://localhost:5000/trackers-info/bulk-update', 
            { IDs: dataDelete },
            { headers: mainHeader() }
        );
...
const Update = async () => {
  setLoads(true); // Set loading state to true
  setLoading(true); // Set additional loading state to true
  ...
  try {
      ...
      const response = await axios.put(
          `http://localhost:5000/trackers-info/${d_ID}`, 
          data,
      ...
      if (response.status === 200) {
          // Fetch updated data and show success alert
          fetchLoad();
          withReactContent(Swal).fire({
              title: <i>Updated!</i>,
              icon: 'success',
  ...
  const deleteLoad = async () => {
    ...
    try {
        const { data: c} = await axios.put(`http://localhost:5000/trackers-info/${deleteid}`,data,{
          headers: mainHeader() 
        })
...
function openAppOrRedirect() {
  ...
  const fetchLoad = async () => {
    ...
    if (data?.data) {
      ...
      setDocID(data?.data?.filter(x => x.isDeleted == false));
      setCell(data?.data);
      // const v = data.map(r=>{
    ...
    <>
      ...
      <div>
        <div className='content-bar'>
          {map ? <>
            {/* <Row><Col><h4 style={{textAlign:'left',fontSize:'16px'}}></h4></Col><Col><Button style={{backgroundColor:'#000',color:'#fff',marginBottom:15}} variant="contained" color="primary" onClick={()=>setColoumn(true)}>NEW TRACKER</Button></Col></Row> */}
            <TrackTruck />
            <ColumnDataTable rows={load} columns={feild} bulkdel={bulkDeleteLoad} openColumn={openColumn}/>
          </> :
            <>
          ...
          {!addlocationdata?.length && cityList.slice(0,50).map(r=><MenuItem value={r?.state}>{r?.state}</MenuItem>)}
                    ...
                    {appointment === 'yes' && <FormControl sx={{ m: 1, width: '25ch' }} xs={12} sm={12} md={6} xl={6}>
                      <TextField
                        focused
                        id="standard-basic"
                        label={shippers ? "Shipping Hours" : "Recieving Hours"}
                        variant="standard"
                        name='hours'
                        type='text'
                        value={hours}
