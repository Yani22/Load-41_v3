...
const ShippersList = () => {
  ...
  const [types, setTypes] = React.useState('shippers')
  ...
  const deleteLoad = async () => {
    setLoading(true)
    try {
      const { docRef } = await axios.delete(`http://localhost:5000/users-details/${deleteid}`,{
        headers: mainHeader()
      })
      setLoading(false)
  ...
  const updateUser = async (e) => {
    ...
    try {
      const { docRef } = await axios.put(`http://localhost:5000/users-details/${d_ID}`,{data:data,type:types},{
        headers: mainHeader()
      })
      setLoading(false)
      fetchUsers()
  ...
  const addTypeOfuser = async (e) => {
    ...
    setLoading(true)
    try {
      const { data: docRef} = await axios.post('http://localhost:5000/users-details',{entries:[{
        data:{
        name: addname,
        city: addcity,
        mobile: addmobile,
  ...
  const updatePop = (id, index) => {
    try {
      const x = docuserID?.filter(x => x?.type === 'shippers' || x?.type === 'sc');
  ...
  const feild = [
    ...
    { field: 'name', headerName: 'Shippers', width: 200, valueGetter: (value, params) => {        
      let result = [];
    ...
    { field: 'address1', headerName: 'Address1', width: 120, valueGetter: (value, params) => {        
      let result = [];
      if (params?.data?.address1 || params?.data['Address 2']) {
        if (params?.data?.address1 || params?.data['Address 2']) {
          result.push(params?.data?.address1 || params?.data['Address 2']);
        }
      } else {
        result = ["Unknown"];
      }
    ...
    { field: 'mobile', headerName: 'Mobile', width: 120, valueGetter: (value, params) => {        
      let result = [];
      if (params?.data?.mobile || params?.data?.Telephone) {
        if (params?.data?.mobile || params?.data?.Telephone) {
          result.push(params?.data?.mobile || params?.data?.Telephone);
        }
      } else {
        result = ["Unknown"];
      }
  ...
  return (
    <div>
      ...
      <div className='content-bar'>
        <ColumnData rows={docuserID?.filter(x => x?.type == 'shippers' || x?.type == 'sc')} columns={feild} openColumn={openColumn}/>
      </div>
      <React.Fragment>
        <Dialog
          open={shippers || edit}
            ...
            <form onSubmit={edit ? updateUser : addTypeOfuser}>
              <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                <Tabs value={values} onChange={handleChangeShippers} aria-label="basic tabs example">
                  <Tab onClick={() => setValues(0)} label={`shippers`} {...a11yProps(0)} />
        ...
        >
          ...
          {!addlocationdata?.length && cityList.slice(0,50).map(r=><MenuItem value={r?.state}>{r?.state}</MenuItem>)}
                    ...
                    <FormControl sx={{ m: 1, width: '25ch' }} xs={12} sm={12} md={6} xl={6}>
                      <FormControlLabel control={<Checkbox value='sc' onChange={handleChange} name='types' checked={types === 'sc' ? true : false} />} label="Add as Consignee" />
                    </FormControl>
